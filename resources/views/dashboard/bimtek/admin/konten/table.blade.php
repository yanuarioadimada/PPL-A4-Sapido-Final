<x-dashboard-layout>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg bg-white">
        ...
        <div class="w-100 mb-4 flex justify-end pr-5">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 py-4 ">
                            ...
                            <form action="{{route('admin.bimtek.update.status',$item->id)}}" method="POST" class="inline mr-4">
                                @csrf
                                @method('PATCH')
                                <input name="status" type="text" value="{{$item->status == 'aktif' ? 'non-aktif' : 'aktif'}}" hidden>
                                <button type="submit" class="font-medium {{$item->status == 'non-aktif' ? 'text-blue-600 dark:text-blue-500 hover:underline' : 'text-red-600 dark:text-red-600'}}">{{$item->status == 'aktif' ? 'ubah non-aktif' : 'ubah aktif'}}</button>
                            </form>
                        
                    </td>
                    <td class="px-6 py-4 ">
                        <a href="{{route('admin.bimtek.edit',$item->id)}}">Edit</a>
                        <a href="{{route('admin.bimtek.show',$item->id)}}">Lihat</a>
                    </td>
                </tr>
                @endforeach 
