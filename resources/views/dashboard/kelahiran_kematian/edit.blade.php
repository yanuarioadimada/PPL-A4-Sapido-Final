<x-app-layout>
    ...
    <form action="{{route('kelahiran-kematian.update',$data->id)}}" method="POST" enctype="multipart/form-data" class="p-10 m-4 bg-white">
        @csrf
        @method('PUT')
        <div class="mb-6">
            ...
            <select name="id_hewan" id="nomor_hewan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="{{$data->id}}" selected>{{$data->hewan->nomor_hewan}}</option>
        ...
        <div class="mb-6">
            <label for="jenis" class="{{$errors->has('jenis') ? 'text-red-700' : 'text-gray-900'}} block mb-2 text-sm font-medium">ID Hewan</label>
