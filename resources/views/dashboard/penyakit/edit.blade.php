<x-app-layout>
    ...
    <form action="{{route('penyakit.update',$penyakit->id)}}" method="POST" enctype="multipart/form-data" class="m-4 p-10 bg-white">
        @csrf
        <div class="mb-6">
            ...
            <select name="id_hewan" id="nomor_hewan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                <option value="{{$penyakit->id}}" selected>{{$penyakit->hewan->nomor_hewan}}</option>
        ...
        <div class="mb-6">
            ...
            <input value="{{$penyakit->gejala}}" name="gejala" type="text" id="nomor_hewan" class="{{$errors->has('gejala') ? 'bg-red-50 border border-red-500 text-red-900 placeholder-red-700 text-sm rounded-lg focus:ring-red-500  focus:border-red-500 block w-full p-2.5  ' : 'bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5'}} " placeholder="Gejala" required />
