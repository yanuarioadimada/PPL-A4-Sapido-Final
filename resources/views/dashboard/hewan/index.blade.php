...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    <div class="p-5"> 
        <h4 class="text-3xl font-bold text-gray-800 mb-2">Data Hewan Ternak</h4>
        <p class="text-xl font-bold text-gray-500">- Admin -</p>
    </div>
    <div class="w-100 mb-4 flex justify-end">
        <a href="{{route('hewan.create')}}" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Tambah Hewan</a>
    </div>
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td class="px-6 py-4">
                    {{$item->induk ? $item->induk->nomor_hewan : ''}}
                </td>
                ...
                <td class="px-6 py-4">
                    {{$item->profile->users->name}}
                </td>
                <td class="px-6 py-4">
                    {{$item->keterangan}}
                </td>
                <td class="px-6 py-4 text-right">
                    <a href="{{route('hewan.edit',$item->id)}}" class="font-medium mr-2 text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                    <form action="{{route('hewan.destroy',$item->id)}}" method="post" class="inline">
                        @csrf
                        @method('delete')
