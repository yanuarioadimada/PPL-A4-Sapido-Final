<x-app-layout>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-10 bg-white m-4">
        <div style="background-image: url({!! asset('img/cover-tumbuh.png') !!}); background-size: cover;">
            <h4 class="text-3xl font-extrabold text-white mb-4 p-10">Tumbuh Kembang</h4>
        </div>
        <div class="w-100 mb-4 flex justify-end">
            <a href="{{route('tumbuh.create')}}" type="button" class="text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Tambah</a>
        </div>
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-900 uppercase font-extrabold bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                <tr>
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 py-4 text-right items-center justify-center">
                        <a target="_blank" class="w-4 items-center" href="{{$item->gambar ? url($item->gambar) : ''}}">
                            @if($item->gambar)
                    ...
                    <td class="px-6 py-4 text-center">
                        ...
                        @else
                        <a href="{{route('tumbuh.edit',$item->id)}}" class="font-medium mr-2 text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
                        <form action="{{route('tumbuh.destroy',$item->id)}}" method="post" class="inline">
                            @csrf
                            @method('delete')
